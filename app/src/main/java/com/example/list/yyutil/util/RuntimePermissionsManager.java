...
public class RuntimePermissionsManager {
    ...
    public void handle(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if (requestCode == this.requestCode) {
            ...
            if (grantedNum < permissions.length) {
                ...
                //如果有write_setting权限，则单独处理，此时requestCallback.requestSuccess()方法在对应的回调方法里调用。
                //保持在一处处理请求结果
    ...
    * */
    private void requestWriteSettingsPermission() {
        Common.toast(activity,"请允许修改系统设置");
    ...
    * */
    @TargetApi(Build.VERSION_CODES.M)
    public void hanleWriteSettings(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_CODE_WRITE_SETTINGS) {
            if (Settings.System.canWrite(activity)) {
                requestCallback.requestSuccess();
            } else {
                Common.toast(activity,"未获得修改系统设置的权限，退出应用");
                activity.finish();
            }
        }
    }
